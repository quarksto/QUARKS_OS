...
import { LeadIntelligenceSidebar } from './LeadIntelligenceSidebar';
import { AdaptiveHeader } from '../components/dashboard/AdaptiveHeader';

const SIDEBAR_COLLAPSED_KEY = 'quarks-hybrid-sidebar-collapsed';
...
export function HybridShell() {
  ...
  const setSidebarCollapsedAndPersist = React.useCallback((valueOrUpdater) => {
  ...
  const refreshPipeline = React.useCallback(() => {
    ...
    api
      ...
      .catch((e) => {
        console.error('HybridShell pipeline load error:', e);
  ...
  useEffect(() => {
    ...
    load();
    // Substituindo intervalo por Real-time Hook
  ...
  useLeadRealtime(null, (event) => {
    if (event.type === 'message_new') {
      // Incrementar contador de não lidos localmente se não for o lead selecionado atualmente
      // (Se for o selecionado, o chat já marca como lida)
      setPipeline(prev => {
        const next = { ...prev };
  ...
  const handleLeadUpdate = async (leadId, data) => {
    try {
      ...
      if (res.data) {
        ...
        // Refresh pipeline to update the list preview
  ...
  return (
    <div className="flex h-screen bg-canvas overflow-hidden">
      <HybridSidebar collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsedAndPersist((c) => !c)} leadCount={leads.length} />
